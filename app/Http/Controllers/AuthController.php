...
class AuthController extends Controller
{
    public function signUp(Request $request){
        ...
        if (!empty($request->confirmEmail) && strpos($request->confirmEmail, '@') == false ) {
            $msg = 'Please enter a valid email';
        }
        ...
        if (!empty($msg) && isset($msg)) {
            ...
            $userCheck = User::where('name', $request->name)->latest('id')->first();
            if (!empty($userCheck)) {
                $username = Str::slug($request->name.$userCheck->id);
            }else{
    ...
    public function signIn(Request $request){
        ...
        if (!empty($request->email)) {
            # code...
            $checkUser = User::where('email', $request->email)->orWhere('username', $request->email)->first();
    ...
    public function socialLogin(Request $request){
        ...
        if (empty($request->social_provider)) {
            $msg = 'Parameters are missing';
        }
        if (empty($request->name)) {
